...
# np.array based functions (non differentiable)


def get_ortho6d_from_R(R):
    """Ortho 6d representation from Rotation matrix
    Credits: interaction network (Chanho)
    Args:
    ...
    if R.ndim == 3:
        obj_num = R.shape[0]
        ortho6d = R[:, :, :2]
        ortho6d = np.swapaxes(ortho6d, 1, 2)
        ortho6d = np.reshape(ortho6d, (obj_num, 6))
    elif R.ndim == 2:
        ortho6d = R[:, :2]
        ortho6d = np.swapaxes(ortho6d, 0, 1)
        ortho6d = np.reshape(ortho6d, (6,))
    return ortho6d



# def normalize_vector_np(v, return_mag=False):
#     mag = np.sqrt(np.sum(v**2, axis=0))  # vector magnitude
#     mag = np.maximum(mag, 1e-8)
#     v_normalized = v / mag
    
#     if return_mag:
#         return v_normalized, mag
#     else:
#         return v_normalized

# def cross_product_np(u, v):
#     i = u[1] * v[ 2] - u[2] * v[1]
#     j = u[2] * v[0] - u[0] * v[2]
#     k = u[0] * v[1] - u[1] * v[0]
    
#     out = np.column_stack((i, j, k))
    
#     return out


# def compute_rotation_matrix_from_ortho6d_np(ortho6d):
#     x_raw = ortho6d[0:3]
#     y_raw = ortho6d[3:6]
    
#     x = normalize_vector_np(x_raw)
#     z = cross_product_np(x, y_raw)
#     z = normalize_vector_np(z)
#     y = cross_product_np(z.squeeze(0), x)
    
#     x = x.reshape(-1, 3, 1)
#     y = y.reshape(-1, 3, 1)
#     z = z.reshape(-1, 3, 1)
    
#     matrix = np.concatenate((x, y, z), axis=2)  # 3*3
#     return matrix
...
def normalize_vector_np(v, return_mag=False):
    ...
    v_mag = np.tile(v_mag.reshape(batch, 1), (1, v.shape[1]))
    v = v / v_mag
...
def cross_product_np(u, v):
    ...
    out = np.column_stack((i, j, k))  # batch*3
    return out
...
def compute_rotation_matrix_from_ortho6d_np(ortho6d):
    ...
    x = normalize_vector_np(x_raw)  # batch*3
    z = cross_product_np(x, y_raw)  # batch*3
    z = normalize_vector_np(z)  # batch*3
    y = cross_product_np(z, x)  # batch*3
