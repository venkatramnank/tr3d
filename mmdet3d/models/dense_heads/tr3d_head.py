...
#     def _init_layers(self, n_classes, in_channels, n_reg_outs = 3):
...
#         center_preds, cls_preds, points = [], [], []
#         for permutation in x.decomposition_permutations:
#             center_preds.append(reg_final[permutation])
#             # cls_preds.append(cls_pred[permutation])
#             points.append(x.coordinates[permutation][:, 1:] * self.voxel_size)

#         return center_preds, points

#     def forward(self, x):
#         center_preds, cls_preds, points = [], [], []

#         for i in range(len(x)):
#             center_pred,  point = self._forward_single(x[i])
...
#         # bbox loss
#         pos_bbox_preds = center_preds[pos_mask]
#         if pos_mask.sum() > 0:
#             pos_points = points[pos_mask]
#             pos_center_preds = center_preds[pos_mask]
#             # pos_bbox_preds = center_preds[pos_mask]
...
#             center_targets = gt_bboxes.gravity_center.to(points.device)[assigned_ids][pos_mask]
...
#     def _loss(self, center_preds, points,
#               gt_bboxes, gt_labels, img_metas):
        
#         center_losses, cls_losses, pos_masks = [], [], []
...
#                 center_losses.append(bbox_loss)
#             cls_losses.append(cls_loss)
#             pos_masks.append(pos_mask)
#         return dict(
#             bbox_loss=torch.mean(torch.cat(center_losses)),
...
#         center_preds,  points = self(x)
#         return self._loss(center_preds, points,
#                           gt_bboxes, gt_labels, img_metas)
