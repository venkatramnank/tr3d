...
from mmdet3d.core.bbox.structures.physion_center3d import Physion3DCenter
...
class PhysionRandomFramePointsDataset(Dataset):
    ...
    def random_select_frames(self, data_files):
        ...
        for data_file in data_files:
            ...
            sampled_frames[data_file[1]].append(
                [data_file[0], data_file[1], data_file[2]])
    ...
    def density_within_boxes(self, pcd, bbox_corners):
        min_coords, max_coords = self._calculate_bounding_box_extents(
            bbox_corners)
    ...
    def get_phys_dict(self, file, img_idx, _file, frame_id):
        ...
        np_cam = np.reshape(np.asarray(
            f_obj[frame_id]['camera_matrices']['camera_matrix_cam0'][:]), (4, 4))
        np_proj_mat = np.reshape(np.asarray(
            f_obj[frame_id]['camera_matrices']['projection_matrix_cam0'][:]), (4, 4))

        # TODO: How to remove the labels(bboxes) which have very small amount of points in them?
        pix_T_cam, _, _ = get_intrinsics_from_projection_matrix(
            np_proj_mat, (img_height, img_width))
        # calib = {
        #     'K': pix_T_cam.astype(np.float32),
        #     'Rt': np_cam.astype(np.float32)
        # }

        pcd_generator = PhysionPointCloudGenerator(
            hdf5_file_path=os.path.join(_file), frame_number=frame_id, plot=False)
        ...
        for seg_id in range(num_segments_in_img):
            ...
            location_list.append(
                f_obj[frame_id]["objects"]["center_cam0"][seg_id])
            ...
            if bbox_3d_dims[2] < 0.1 or bbox_3d_dims[0] < 0.1 or bbox_3d_dims[1] < 0.1:
                continue  # based on height/width/length remove carpet

            dimensions_list.append(
                [bbox_3d_dims[1], bbox_3d_dims[2], bbox_3d_dims[2]])

            yaw = math.atan2(2.0*(y*z + x*y), w*w + x*x - y*y - z*z)
            heading_ang.append(yaw)

            # NOTE: using center only
            gt_boxes_upright_depth = [center_x, center_y, center_z]

            if len(gt_boxes_upright_depth) == 0:
                import pdb
                pdb.set_trace()
    ...
    def get_gt_data_info(self, index):
        ...
        with h5py.File(raw_info[1], 'r') as file:
            info = self.get_phys_dict(
                file, raw_info[0], raw_info[1], raw_info[2])
        if info is None:
            return
    ...
    def center_eval_physion(self, gt_annos, dt_annos, metric, label2cat, logger=None,
                            box_type_3d=None,
                            box_mode_3d=None):
        ...
        mse = eval_center(pred, gt)
        import pdb; pdb.set_trace()
        print("Class   |  MSE")
        print("----------------")
        for class_name, mse_vals in mse.items():
            print(f"{label2cat[class_name]}|  {mse_vals}")
        return mse
    ...
    def get_ann_info(self, index):
        ...
        if info['annos']['gt_num'] != 0:
            ...
            gt_bboxes_3d = np.zeros((0, 3), dtype=np.float32)
            gt_labels_3d = np.zeros((0, ), dtype=np.int64)

        ######################################################################################################################################################
        # Testing the new physion box structure
        gt_bboxes_3d = Physion3DCenter(
    ...
    def evaluate(self,
        ...
        import pdb
        pdb.set_trace()
        ret_dict = self.center_eval_physion(
            gt_annos,
            results,
            iou_thr,
            label2cat,
    ...
    def copy(self):
        copied_dataset = PhysionRandomFramePointsDataset(
